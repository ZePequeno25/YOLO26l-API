...
Provides status, diagnostic metrics, and debugging helper endpoints
for file uploads and object detection models.
"""
import logging
import os
...
async def get_status():
    """
    Returns the online status and configuration details of the backend service.
    """
    return {
        "status": "online",
...
async def get_classes():
    """
    Returns the list of classes supported/detectable by the YOLO model.
    """
...
async def debug_upload_raw(
    ...
    try:
        ...
        return {
            ...
            "warning": (
                "Este é um dump bruto do que foi recebido. "
                "Use para diagnosticar problemas de upload."
            ),
            "data": debug_info
        }

    except Exception as e:
        logger.error("Erro no debug: %s", str(e), exc_info=True)
...
async def test_upload(
    ...
    try:
        ...
        logger.error("Erro no teste de upload: %s", str(e), exc_info=True)
        raise HTTPException(status_code=500, detail=str(e)) from e
...
async def detection_debug(
    ...
    try:
        ...
        model_info = {
            ...
            "all_class_names": service.model.names,
            "class_index_to_name_mapping": {
                str(idx): name for idx, name in service.model.names.items()
            }
        }

        detection_result = {
            "detected_classes": result["class_counts"],
        ...
        return {
            ...
            "hint": (
                "Use 'model_info.class_index_to_name_mapping' "
                "para verificar o mapeamento de classes"
            )
        }

    except Exception as e:
        logger.error("Erro na detecção debug: %s", str(e), exc_info=True)
        raise HTTPException(status_code=500, detail=str(e)) from e