...
class SlidingWindowRateLimiter:
    """
    Sliding window rate limiter reusable as a FastAPI dependency.
    """
    ...
    def __call__(self, request: Request) -> None:
        """
        Executes the rate limiting check on incoming requests.
        """
        ...
        with self._lock:
            ...
            logger.debug(
                "Rate limit: ip=%s path=%s hits=%d/%d",
                ip, request.url.path, count, self.max_hits
            )
