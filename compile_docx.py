"""
Script to compile Markdown chapters of the TCC into a DOCX template.
Applies ABNT formatting standards recursively.
"""
...
try:
    import docx
    # Monkeypatch docx to handle decimal Twips measures
    try:
        ...
        for name, obj in list(st.__dict__.items()):
            if isinstance(obj, type) and hasattr(obj, 'convert_from_xml'):
                ...
                def make_wrapper(orig_fn):
                    """Creates a wrapper to safely convert XML float strings to integer twips."""
                    @classmethod
...
def append_markdown_file(doc, file_path):
    """
    Reads a Markdown file and appends its formatted contents to the Word Document.
    """
    ...
    while i < len(lines):
        ...
        # If inside a code block, format as preformatted text
        # (Courier New, single spacing, indented)
        if in_code_block:
            p = doc.add_paragraph()
            ...
            # Use the original unstripped line to preserve whitespace
...
def main():
    """Main function to parse revision markdown files and update the docx file."""
    ...
    for idx, p in enumerate(doc.paragraphs):
        ...
        if txt in ("1 introdução", "introdução", "# introdução"):
            start_idx = idx
            break
    ...
    if start_idx == -1:
        ...
        for idx, p in enumerate(doc.paragraphs):
            txt = p.text.strip().lower()
            if "introdução" in txt and (
                txt.startswith("1") or p.style.name.startswith("Heading")
            ):
                start_idx = idx
                break
    ...
    # 2. Remover todos os elements da introducao ate o final da body tree
